...
get_new_nir<-function() {
  ...
  new_nir<-nir_2022 %>% 
    rename(Sector=Category,Sub.sector=Sub.category,
           Sub.sub.sector=Sub.sub.category)%>%
    ...
    mutate(sector=factor(sector),
           sector=fct_collapse(sector,"Inventory Total"=
                                 c("Territories Inventory Total",
                                   "Territory Inventory Total",
                                   "Territorial Inventory Total",
                                   "Provincial Inventory Total"))
    )%>%
...
ggplot(totals)+
  ...
  scale_colour_manual("",values=c("black",colors_ua10()[-c(3,5)]),
                      guide = "legend")+
...
pop_proj<-read_csv("pop_proj.csv",col_types = cols(.default = "c")) %>%
  ...
  mutate(prov=as.factor(region),pop=as.numeric(gsub(",","",pop)),
         ...
         prov=fct_collapse(prov,
                           #"TERR" = c("NT", "NU","YT","NT & NU")
                           #,"ATL" = c("NL", "NB","NS","PE")
                           "OTHER" = c("NT", "NU","YT","NT & NU", "NL", "NB","NS","PE", "BC", "QC", "MB")
         ))%>%
...
proj_data$prov_f = factor(proj_data$prov, levels = c('AB','ON','QC','SK','BC',
                                                     'ATL','MB','TERR'))

prov_proj<-ggplot(filter(proj_data,emissions>0 & 
                           scenario%in% c("NIR 2022", "2021 Reference Case") & 
                           prov !="Canada" & year <=2020 & year >=2005))+
  geom_area(aes(year,emissions,fill=sector),
            color="black",position = "stack",size=0.1,alpha=.4)+
  geom_area(data=filter(proj_data,emissions>0 & 
                          scenario%in% c("NIR 2022", "2021 Reference Case") & 
                          prov !="Canada" & year<=2020 & year >=2005),
            aes(year,emissions,fill=sector),
            color="black",position = "stack",size=0.1,alpha=.8)+
  geom_hline(yintercept=0) +
  facet_wrap( ~ prov_f,nrow = 1)+
  scale_x_continuous(breaks=pretty_breaks())+
  ...
  scale_fill_manual("",values = c("grey10","grey25","dodgerblue","grey40",
                                          "grey55","grey70","grey85"),
                                          guide = "legend")+
  #scale_fill_grey("",guide = "legend",start = 0.9,end=0.05)+
  ...
  theme(
    ...
    axis.ticks = element_line(size = rel(1),colour="black"),#"#D8D8D8"),    # Change x axis ticks only
    ...
    axis.text.x = element_text(angle=90,hjust = .5,vjust=.5,size=15),#rel(.7)),
    strip.text = element_text(size=rel(.7), face = "bold"),
...
proj_db <- proj_data %>%
  filter(sector=="Electricity",
         ...
         scenario%in% c("NIR 2022", "2021 Reference Case"),
         prov !="Canada" & year <=2020 & year >=2005) %>%
  mutate(prov=fct_collapse(prov,
                           #"TERR" = c("NT", "NU","YT","NT & NU")
                           #"ATL" = c("NL", "NB","NS","PE")
                           "OTHER" = c("TERR", "BC","QC","MB")))

proj_db$prov1 = factor(proj_db$prov, levels=c("AB","ON","ATL","SK","OTHER"))

prov_proj1<-ggplot(proj_db)+
  geom_area(aes(year,emissions),
            fill="black",size=0.1,alpha=.8)+
  geom_hline(yintercept=0) +
  facet_wrap( ~ prov1,nrow = 1)+
  scale_x_continuous(breaks=pretty_breaks())+
  scale_y_continuous(breaks=pretty_breaks(),
                     expand=c(0,0)) +
  scale_linetype_manual("",values=c("11","22","33"),guide = "legend")+
  blake_theme()+ 
  labs(x=NULL,y=#bquote("Annual Electricity Generation Emissions (MtCO"[2]e)),
         expression(atop('Annual Electricity Generation', paste('Emissions  '*'(MtCO'[2]*'e)'))),
       caption="Source: Environment and Climate Change Canada.",
       NULL) +
  theme(panel.spacing = unit(.75,"lines"))+
  theme(
    plot.margin = margin(t = .05, r = .1, b = .025, l = .9,unit= "cm"),
    axis.ticks = element_line(size = rel(1),colour="black"),#"#D8D8D8"),    # Change x axis ticks only
    axis.ticks.length = unit(5, "pt"), # Change the length of tick marks
    legend.position = "right",
    legend.key.width = unit(1.05,"cm"),
